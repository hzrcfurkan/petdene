...
export function AppointmentImport() {
	...
	const handleImport = async () => {
		...
		try {
			...
			// 2. Validate each row
			// 3. Map pet names to pet IDs, service titles to service IDs
			...
			// await mutationFetcher("/api/v1/appointments/import", {
			//   method: "POST",
		...
		<div className="space-y-6">
			<Card>
				<CardHeader>
					...
					<CardDescription>
						Upload a CSV file with appointment data. Required columns: petName (or petId), serviceTitle (or serviceId), date, time
					</CardDescription>
				</CardHeader>
									...
									<TableBody>
										{preview.map((row, index) => (
											<TableRow key={index}>
												...
												<TableCell>{row.serviceTitle || row.serviceId || "N/A"}</TableCell>
												<TableCell>{row.date || "N/A"}</TableCell>
												<TableCell>{row.time || "N/A"}</TableCell>
												<TableCell>{row.staffEmail || "N/A"}</TableCell>
											</TableRow>
										))}
									</TableBody>
								</Table>
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>Required columns and format for appointment import</CardDescription>
				</CardHeader>
				<CardContent>
					<div className="space-y-2 text-sm">
						...
						<ul className="list-disc list-inside space-y-1 ml-4">
							...
							<li><code>serviceTitle</code> or <code>serviceId</code> - Service name or ID</li>
							<li><code>date</code> - Appointment date (YYYY-MM-DD)</li>
							<li><code>time</code> - Appointment time (HH:MM)</li>
						</ul>
						<p className="mt-4"><strong>Optional columns:</strong></p>
						<ul className="list-disc list-inside space-y-1 ml-4">
							<li><code>staffEmail</code> - Staff member email</li>
